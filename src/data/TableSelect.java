...
public class TableSelect {
	...
	public String dataPath =System.getProperty("user.home");
		...
		try {
			...
			if (args.length == 0)
				conn = DriverManager.getConnection(protocol
						+ "animalsurvey;create=true", props);
			else
				conn = DriverManager.getConnection(protocol + dataPath
	...
	void go(String[] args) throws SQLException {
		...
		if (args.length != 0)
			...
			rs = s
					.executeQuery("SELECT * FROM animalsurvey ORDER BY surveyoption");
		rmd = rs.getMetaData();
